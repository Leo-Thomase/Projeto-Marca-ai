...
$query = $pdo->query("SELECT * FROM $tabela where data_venc >= '$dataInicial' and data_venc <= '$dataFinal' and tipo = 'Conta' and pago LIKE '$status' ORDER BY pago asc, data_venc asc");
...
<td>
		...
		<big><a href="#" onclick="mostrar('{$descricao}', '{$valorF}', '{$data_lancF}', '{$data_vencF}',  '{$data_pgtoF}', '{$nome_usuario_lanc}', '{$nome_usuario_pgto}', '{$tumb_arquivo}', '{$foto}', '{$nome_func}', '{$telefone_func}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
