...
@session_start();
$data = array(
    'appkey' => '78ef6a29-57f3-4415-a2c0-2d19584650a5',  // Sua chave de aplicação
...
if ($response === false) {
    ...
    if (isset($response_data['id'])) {
        ...
        $id = $response_data['id'];
        $query = $pdo->prepare("INSERT INTO agendamentos SET id = :id, funcionario = :funcionario, cliente = :cliente, hora = :hora, data = :data, usuario = '0', status = 'Agendado', obs = :obs, data_lanc = curDate(), servico = :servico, hash = :hash");
        ...
        $query = $pdo->prepare("INSERT INTO agendamentos SET funcionario = :funcionario, cliente = :cliente, hora = :hora, data = :data, usuario = '0', status = 'Agendado', obs = :obs, data_lanc = curDate(), servico = :servico");
        $query->bindValue(":funcionario", $funcionario);
        $query->bindValue(":cliente", $id_cliente);
        $query->bindValue(":hora", $hora);
        $query->bindValue(":data", $data_agd);
